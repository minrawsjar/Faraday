...
export function ReservePanel() {
  ...
  const needsApproval = mode === "deposit" && (allowance ?? 0n) < parsedAmount;
  ...
  return (
    <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <Stat label="Total Reserve" value={`$${formatUnits(totalReserve ?? 0n, 6)}`} />
        <Stat label="Liquid USDC" value={`$${formatUnits(liquidUsdc, 6)}`} />
        <Stat label="In USYC" value={`${formatUnits(usycShares, 6)} shares`} dim />
      </div>
      ...
      <div className="space-y-3">
        ...
        {mode === "deposit" && usdcBalance !== undefined && (
          <p className="text-xs text-gray-500">
            Wallet balance: {parseFloat(formatUnits(usdcBalance, 6)).toFixed(2)} USDC
          </p>
        )}
